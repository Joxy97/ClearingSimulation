...
class GrayQuantizer:
    ...
    def decode_dataframe(self, binary_df: pd.DataFrame) -> pd.DataFrame:
        ...
        Args:
            binary_df: DataFrame with columns like 'AAPL_g0', 'AAPL_g1', ...
        ...
        # Identify assets from column names
        assets = []
        for col in binary_df.columns:
            if col.endswith("_g0"):
                asset = col[:-3]
                if asset in self.edges:
                    assets.append(asset)

        decoded = pd.DataFrame(index=binary_df.index, columns=assets, dtype=np.float64)

        for asset in assets:
            bit_cols = [f"{asset}_g{b}" for b in range(self.nbits)]
            bits = binary_df[bit_cols].values
            decoded[asset] = self.from_gray(bits, asset)
